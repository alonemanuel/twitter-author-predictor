...
class SuperLearner():
	...
	def __init__(self, models, prep):
		...
		self.classifier_name = 'Super Learner'
	...
	def predict(self, X):
		...
		predictions = []
		for model in self.models:
			predictions.append(model.predict(prep_X))
		prediction_vec = np.stack(predictions, axis='columns')
		# y_pred = prec
