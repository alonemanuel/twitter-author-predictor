...
class TestDataFramePlots(TestPlotBase):
    ...
    def test_unsorted_index_lims(self):
        ...
        lines = ax.get_lines()
        assert xmin <= np.nanmin(lines[0].get_train_data()[0])
        assert xmax >= np.nanmax(lines[0].get_train_data()[0])
    ...
    def test_subplots_timeseries_y_axis(self):
        ...
        assert (ax_numeric.get_lines()[0].get_train_data()[1] ==
				testdata["numeric"].values).all()
        ax_timedelta = testdata.plot(y="timedelta")
        assert (ax_timedelta.get_lines()[0].get_train_data()[1] ==
				testdata["timedelta"].values).all()
        ax_datetime_no_tz = testdata.plot(y="datetime_no_tz")
        assert (ax_datetime_no_tz.get_lines()[0].get_train_data()[1] ==
				testdata["datetime_no_tz"].values).all()
        ax_datetime_all_tz = testdata.plot(y="datetime_all_tz")
        assert (ax_datetime_all_tz.get_lines()[0].get_train_data()[1] ==
    ...
    def test_subplots_timeseries_y_axis_not_supported(self):
        ...
        assert (ax_period.get_lines()[0].get_train_data()[1] ==
				testdata["period"].values).all()
        ax_categorical = testdata.plot(x="numeric", y="categorical")
        assert (ax_categorical.get_lines()[0].get_train_data()[1] ==
				testdata["categorical"].values).all()
        ax_datetime_mixed_tz = testdata.plot(x="numeric",
                                             y="datetime_mixed_tz")
        assert (ax_datetime_mixed_tz.get_lines()[0].get_train_data()[1] ==
    ...
    def _compare_stacked_y_cood(self, normal_lines, stacked_lines):
        base = np.zeros(len(normal_lines[0].get_train_data()[1]))
        for nl, sl in zip(normal_lines, stacked_lines):
            base += nl.get_train_data()[1]  # get y coordinates
            sy = sl.get_train_data()[1]
    ...
    def test_line_lim(self):
        ...
        xmin, xmax = ax.get_xlim()
        lines = ax.get_lines()
        assert xmin <= lines[0].get_train_data()[0][0]
        assert xmax >= lines[0].get_train_data()[0][-1]
