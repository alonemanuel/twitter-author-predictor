...
class TestSeriesPlots(TestPlotBase):
    ...
    def test_ts_line_lim(self):
        ...
        lines = ax.get_lines()
        assert xmin <= lines[0].get_train_data(orig=False)[0][0]
        assert xmax >= lines[0].get_train_data(orig=False)[0][-1]
        tm.close()
    ...
    def test_ts_area_lim(self):
        ...
        xmin, xmax = ax.get_xlim()
        line = ax.get_lines()[0].get_train_data(orig=False)[0]
        assert xmin <= line[0]
        assert xmax >= line[-1]
        tm.close()
    ...
    def test_unsorted_index_xlim(self):
        ...
        xmin, xmax = ax.get_xlim()
        lines = ax.get_lines()
        assert xmin <= np.nanmin(lines[0].get_train_data(orig=False)[0])
        assert xmax >= np.nanmax(lines[0].get_train_data(orig=False)[0])
